...
(function() {
    ...
    unsafeWindow.XMLHttpRequest.prototype.open = function(method, url) {
        ...
        if (url.includes('api/user/info')) {
            // 用户信息
            user.vip === 1 ? this.addEventListener('readystatechange', function() {
                if (this.readyState === 4) {
                    ...
                    user.ad === 1 ? res.data.IsShowAdvertisement = false : "" // 广告
                    if (user.endtime) {
                    ...
                    user.name ? res.data.Nickname = user.name : ""
                    user.photo ? res.data.HeadImage = user.photo : ""
                    user.mail ? res.data.Mail = user.mail : ""
                    user.phone ? res.data.Passport = Number(user.phone) : ""
                    user.id ? res.data.UID = Number(user.id) : ""

                    if (user.debug === 1 || user.debug === 2) console.log("【123 云盘解锁】Hook XHR", "\n请求地址:", url, "\n原始回复:", oriRes, "\n修改回复:", res)
    ...
    unsafeWindow.XMLHttpRequest.prototype.setRequestHeader = function(header, value) {
        ...
        if (!!url && url.includes('download')) {
            ...
            if (resName.toLowerCase() in headers) {
                ...
                if (user.debug === 1 || user.debug === 3) console.log("【123 云盘解锁】Hook XHR", "\n请求地址:", url, "\n请求头项:", {
                    [oriResName]: oriResValue
                }, "\n修改头项:", {
                    [resName]: resValue
                }, "\n随机项目:", info)
